...
Feature: An administrator wants to export the files of his user using
  ...
  Scenario: An deleted file should be contained in an export
    ...
    When user "user0" deletes file "/testFolder/trashbinFile.txt" using the WebDAV API
    Then as "user0" file "trashbinFile.txt" should exist in the trashbin
    When user "user0" is exported to path "/tmp/fooSomething" using the occ command
